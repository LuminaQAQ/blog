...
<template>
  <main class="main-user">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          {{ scope.row.ltime?.replace(/^(\d{4})-(\d{2})-(\d{2}).*/, '$1-$2-$3') }}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async init() {
      await this.fetchUserInfoList({ TOKEN: this.token, index: (this.pageIndex - 1) * 2, size: this.pageSize });
    },
    // #endregion
    // ------- end -------
  },
  created() {
    ...
    this.fetchUserInfoList({ TOKEN, index: this.pageIndex - 1, size: this.pageSize });
    this.token = TOKEN;
  },
};
</script>